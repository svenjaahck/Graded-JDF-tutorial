...
public class Downloader extends Thread {
	...
	public void download() {
		...
		try {
			synchronized (output) {
				oracle(); //Check if content is fine for first 3 bytes
				output.close();
			}
		} catch (IOException e) {
...
//		pw.println();
	...
	public void oracle() {
		try {
			outputFileName = URLDecoder.decode(filename, "UTF-8").substring(filename.indexOf("/") + 1);
			RandomAccessFile contents = new RandomAccessFile(new File(outputFileName), "r");
			byte[] toCheck = new byte[3]; //checking for a, b, c
			contents.read(toCheck, 0, 3);
			...
			die("Cannot open, issue with file", e);
		}
	}
